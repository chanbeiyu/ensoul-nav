...
var menu = {
    "open": {
        name: "打开", icon: "fa-external-link", callback: function (key, opt) {
    ...
    "delete": {
        name: "删除", icon: "delete", callback: function () {
            var link_id = $(this).attr('id');
            link_id = link_id.replace('id_', '');
            mdui.confirm('确认删除？'
                , function () {
    ...
    "sep1": "---------",
    "qrcode": {
        name: "二维码", icon: "fa-qrcode", callback: function (data, status) {
    ...
    "copy": {
        name: "复制链接", icon: "copy", callback: function () {
            let url = $(this).attr('link-url');
            url = url.substr(0, 11) == "./index.php" ? baseUrl + url.slice(11) : url;
            if (isSupported) {
