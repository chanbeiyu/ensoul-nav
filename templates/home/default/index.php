...
<head>
    ...
    <?php if ($referrer == 'overall' && $site['link_model'] == 'direct') {
    ...
    <link rel="stylesheet"
          href="<?php echo $theme_dir ?>/static/style<?php echo $theme_config['CardNum']; ?>.css?v=<?php echo $theme_ver; ?>">
    <link rel="shortcut icon" href="<?php echo $favicon; ?>">
    <style>
        <?php  $SBC = $theme_config['SidebarBackgroundColor']; if( empty($night)  ) {?>
        /*配色*/
        .mdui-theme-primary-indigo .mdui-color-theme {
            background-color: <?php echo $theme_config['HeadBackgroundColor'];?> !important;
        }

        .mdui-loaded .mdui-drawer {
        <?php echo(empty($SBC)?'':'background-color:'.$SBC.'!important;');?>
        }

        .HFC {
            color: <?php echo $theme_config['HeadFontColor'];?> !important;
        }

        .CBC {
            background-color: <?php echo $theme_config['CardBackgroundColor'];?> !important;
        }

        .OBC {
            background-color: <?php echo $theme_config['OtherBackgroundColor'];?> !important;
        }

        .CFC {
            color: <?php echo $theme_config['CategoryFontColor'];?> !important;
        }

        .TFC {
            color: <?php echo $theme_config['TitleFontColor'];?> !important;
        }

        .DFC {
            color: <?php echo $theme_config['DescrFontColor'];?> !important;
        }
        ...
        .mdui-card-primary {
            padding-top: <?php if($DescrHeight == 0){echo '8px';}else{echo '16px';} ;?>;
        }
    </style>
...
<body
    class="mdui-drawer-body-left mdui-appbar-with-toolbar mdui-theme-primary-indigo mdui-theme-accent-pink mdui-loaded OBC <?php echo $night; ?>">
<!--导航工具-->
<header class="mdui-appbar mdui-appbar-fixed">
    <div class="mdui-toolbar mdui-color-theme">
        <span class="mdui-btn mdui-btn-icon mdui-ripple mdui-ripple-white HFC"
        ...
        <?php if ($WeatherPosition == 1) {
            echo '<div id="he-plugin-simple"></div>';
        } ?>
        <a class="mdui-hidden-xs mdui-btn mdui-btn-icon" id="config" title="主题设置" <?php if (!is_login) {
            echo 'style="display:none;"';
        } ?>><i class="mdui-icon material-icons HFC">&#xe40a;</i></a>
        <!-- 新版搜索框END -->
    </div>
</header>
<!--导航工具END-->
...
<?php } ?>
<!-- 返回顶部按钮 -->
<div class="top mdui-shadow-10"><a href="javascript:;" title="返回顶部" onclick="gotop()"><i
            class="mdui-icon material-icons">arrow_drop_up</i></a></div>
<!--左侧抽屉导航-->
<div class="mdui-drawer" id="drawer">
    <ul class="mdui-list">
        ...
        <a href="./index.php?c=admin&u=<?php echo $u ?>">
            <li class="mdui-list-item mdui-ripple">
                <div class="mdui-list-item-content category-name CFC"><i class="fa fa-user-circle"></i>系统管理</div>
            </li>
        </a>
    </ul>
...
<div class="mdui-container">
    <?php if ($WeatherPosition == 2) {
        echo '<div style="position:fixed;z-index:1000;right:0px;width:160px;padding-right:0px;"><div id="he-plugin-simple"></div></div>' . "\n";
    } ?>
    <div class="mdui-row">
        <!-- 遍历分类目录 -->
        <?php foreach ($categorys as $category) {
            ...
            foreach ($links as $link) {
                ...
                <div class="mdui-col-lg-3 mdui-col-md-4 mdui-col-xs-12 link-space" id="id_<?php echo $link['id']; ?>"
                     link-title="<?php echo $link['title']; ?>" link-url="<?php echo $link['url']; ?>">
                    ...
                    <div class="mdui-card link-line mdui-hoverable CBC">
                        ...
                        <?php } ?>
                        <!-- 角标END -->
                        <a class="TFC" href="<?php echo $link['url']; ?>" target="_blank" <?php echo $protectA; ?>
                           title="<?php echo $link['description']; ?>">
                            <div class="mdui-card-primary">
                                <div class="mdui-card-primary-title link-title">
                                    <img src="<?php echo $link['ico']; ?>" alt="HUAN" width="16px"
                                         height="16px" <?php echo $protectIMG; ?>>
                                    <span class="link_title"><?php echo $link['title']; ?></span>
                                </div>
                            </div>
                            <?php echo $CLBT; ?>
                            <!-- 卡片的内容end -->
                            <div class="mdui-card-content mdui-text-color-black-disabled DFC" style="padding-top:0px;">
                                <span class="link-content"><?php echo $link['description']; ?></span>
                            </div><?php echo $CLALL; ?>
                    </div>
                    <!--卡片END-->
                </div>
