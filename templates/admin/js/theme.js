    ...
    $(".layui-btn-group .layui-btn").click(function () {
        ...
        if (type === 'dw' || type === 'up') { //下载或更新
            if (data.desc != null && data.desc.length != 0) { //存在描述时弹窗显示描述
                layer.open({
                    title: data.name, content: data.desc, btn: ['下载', '取消']
...
function theme_detail(data) {
    layer.open({
        ...
        shadeClose: true,
        resize: false,
        title: data.name + ' - 主题详情',
        area: ['60%', '59%'],
        content: '<body class="layui-fluid"><div class="layui-row" style = "margin-top:1em;"><div class="layui-col-sm9" style = "border-right:1px solid #e2e2e2;"><div style = "margin-left:1em;margin-right:1em;"><img src="' + data.screenshot + '" alt="" style = "max-width:100%;"></div></div><div class="layui-col-sm3"><div style = "margin-left:1em;margin-right:1em;"><h1>' + data.name + '</h1><p>描述：' + data.description + '</p><p>版本：' + data.version + '</p><p>更新时间：' + data.update + '</p><p>作者：' + data.author + '</p><p>主页：<a style = "color:#01AAED;" href="' + data.homepage + '" target="_blank" rel = "nofollow">访问主页</a></p></div></div></div></body>'
    });
