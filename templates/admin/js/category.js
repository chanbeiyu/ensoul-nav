layui.use(['form', 'table', 'treetable', 'xIcon'], function () {
    ...
    cols = [[
        ...
        , {
            field: 'add_time', title: '添加时间', width: 166, templet: function (d) {
    ...
    form.on('switch(property)', function (obj) {
        ...
        $.post(get_api('write_category', 'property_sw'), {
            'cid': obj.elem.value,
            'property': sw ? '1' : '0'
    ...
    form.on('switch(status)', function (obj) {
        ...
        $.post(get_api('write_category', 'status_sw'), {
            'cid': obj.elem.value,
            'status': sw ? '1' : '0'
        }, function (data, status) {
            if (data.code == 1) {
    ...
    table.on('tool(table)', function (obj) {
        if (limit) {
            layer.tips("^_^ 请先保存排序.", "#btn-save", {tips: [3, "#3595CC"], time: 3000});
            return;
        }
        var data = obj.data;
        if (obj.event === 'del') {
            ...
            $("#save").text('更新');
            $("#fid option[value='" + data.cid + "']").attr("disabled", "disabled");
            disabled_id = data.cid; //禁止选择自己
            ...
            layer.alert('名称: ' + pwds['pid_' + data.pwd_id].name + '<br>密码: ' + pwds['pid_' + data.pwd_id].pwd, {
                icon: 4,
                title: data.name + ' - 访问密码',
                anim: 2,
                closeBtn: 0
            });
        }
    ...
    function updateSortData() {
        ...
        if (datas.length > 1) {
            if (type == '&type=all') {
                ...
                for (var i = 0; i < datas.length; i++) {
                    new_datas.push([datas[i].cid, i + 1])
                }
            }
        }
