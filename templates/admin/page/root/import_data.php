...
<script>
    layui.use(['layer', 'form', 'miniTab'], function () {
        ...
        $('.import_data').on('click', function () {
            index = layer.prompt({
                formType: 3, value: '', title: '请输入文件名:', shadeClose: false, "success": function () {
        ...
        function import_data(value) {
            ...
            $.post(get_api('other_root', 'import_data'), {
                'step': 1,
                'file': $("input.layui-layer-input").val()
                ...
                if (i >= datas.count) {
                    $.post(get_api('other_root', 'import_data'), {'step': 3}, function (data, status) {
                        if (data.code == 1) {
                            ...
                            layer.closeAll();
                            layer.alert(data.msg || "未知错误,请联系开发者!", {
                                icon: 5,
                                title: '导入失败',
                                anim: 2,
                                shadeClose: false,
