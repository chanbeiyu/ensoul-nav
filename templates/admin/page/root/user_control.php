...
<script type="text/html" id="user_tool">
    <div class="layui-btn-group">
        ...
        <button class="layui-btn layui-btn-sm"
                lay-event="register" <?php echo $global_config['RegOption'] == 0 ? 'style = "display:none;"' : '' ?> >
            注册账号
        </button>
    ...
    layui.use(['table', 'layer', 'form'], function () {
        ...
        var cols = [[
            ...
            , {
                ...
                sort: true,
                event: 'login_entry',
                style: 'cursor: pointer;color: #3c78d8;'
            }
        ...
        table.on('toolbar(table)', function (obj) {
            ...
            //取被选中的链接ID
            tableIds = checkStatus.data.map(function (value) {
                return value.ID;
            });
            tableIds = JSON.stringify(tableIds);
            table_Users = checkStatus.data.map(function (value) {
                return value.User;
            });
            console.log(event);
            //删除
            if (event == 'Del') {
                layer.alert("您将删除以下账号:<br />" + table_Users, {
                    icon: 3,
                    title: '确认操作',
                    anim: 2,
                    closeBtn: 0,
        ...
        table.on('tool(table)', function (obj) {
            ...
            if (obj.event == 'set_pwd') {
                layer.prompt({formType: 3, value: '', title: '请输入新密码'}, function (value, index, elem) {
                    $.post(get_api('write_user_info', 'set_pwd'), {
                        ID: data.ID,
                        new_pwd: $.md5(value)
                ...
                layer.prompt({formType: 3, value: '', title: '请输入新邮箱'}, function (value, index, elem) {
                    $.post(get_api('write_user_info', 'set_email'), {
                        ID: data.ID,
                        new_email: value
                    }, function (data, status) {
                        if (data.code == 1) {
        ...
        form.on('submit(save_UserGroup)', function (data) {
            ...
            $.post(get_api('write_user_info', 'set_UserGroup'), {
                ID: IDs,
    ...
    <form class="layui-form layuimini-form" lay-filter="set_UserGroup">
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                ...
                <button class="layui-btn layui-btn-normal" lay-submit lay-filter="save_UserGroup" id='save_UserGroup'>
                    保存
                </button>
            </div>
        </div>
...
注:错误的规则可能会造成程序异常,如需帮助请联系技术支持QQ:271152681或技术交流群695720839
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn layui-btn-warm" type="button" id="close">关闭</button>
                <button class="layui-btn layui-btn-normal" lay-submit lay-filter="save_username_retain"
                        id='save_username_retain'>保存
                </button>
            </div>
        </div>
    </form>
