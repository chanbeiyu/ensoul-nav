        ...
        <div class="layui-col-lg8 layui-col-md-offset2">
            ...
            <form class="layui-form layui-form-pane">
                ...
                <blockquote class="layui-elem-quote"
                            style="margin-top: 10px;border-left: 5px solid #FF5722; color: #FF5722;" id='guide'>
                    第一步:请上传数据,支持: db3 / html 格式(最大10M),使用前请参考<a
                        href="https://gitee.com/tznb/TwoNav/wikis/pages?sort_id=7968661&doc_id=3767990" target="_blank"
                        rel="nofollow">帮助文档</a></blockquote>
                ...
                <div class="layui-form-item" style="display:none;" id='AutoClass'>
                    ...
                    <div class="layui-input-inline" style="width: 70px;">
                        <input lay-filter="AutoClass" type="checkbox" name="AutoClass" value="1" lay-skin="switch"
                               lay-text="是|否">
                    </div>
                ...
                <div class="layui-form-item" style="display:none;" id='2Class'>
                    ...
                    <div class="layui-input-inline" style="width: 70px;">
                        <input lay-filter="2Class" type="checkbox" name="2Class" value="1" lay-skin="switch"
                               lay-text="是|否">
                    </div>
            ...
            <blockquote class="layui-elem-quote" style="margin-top: 10px;border-left: 5px solid #5FB878; color: #333;">
                1.按需选择参数 > 将下方蓝色的「一键添加」拖拽到浏览器书签栏(收藏夹)<br/>2.点击书签栏中的一键添加,即可将正在浏览的页面快速添加到TwoNav
            </blockquote>
            <form class="layui-form layui-form-pane" id="one">
                ...
                <div class="layui-form-item">
                    <button class="layui-btn layui-btn-primary layui-border-black" type="button" id="view_script">
                        脚本预览
                    </button>
                    <a class="layui-btn layui-btn-primary layui-border-blue" style="cursor:pointer;color: #1e9fff;"
                       title="将此链接拖拽到书签栏" id="one-click-add" href=''>一键添加</a>
                </div>
            </form>
            <!--书签导出-->
            <fieldset class="layui-elem-field layui-field-title" style="margin-top: 30px;">
            ...
            <blockquote class="layui-elem-quote" style="margin-top: 30px;">
                <a target="_blank" style="cursor:pointer;" rel="nofollow" data="db3" class="export_data">导出SQLite (
                    可导入: OneNav Extend / TwoNav ) &nbsp;>>&nbsp; 仅包含分类和链接!</a>
            </blockquote>
            <blockquote class="layui-elem-quote" style="margin-top: 10px;">
                <a target="_blank" style="cursor:pointer;" rel="nofollow" data="html" class="export_data">导出HTML (
                    可导入: 浏览器 / OneNav / OneNav Extend / TwoNav ) </a>
            </blockquote>
            ...
            <blockquote class="layui-elem-quote"
                        style="margin-top: 10px;border-left: 2px solid #FF5722; color: #FF5722;">1.备份数据库仅保存最近20份数据<br/>2.该功能仅辅助备份使用，无法确保100%数据安全，因此定期对整个站点打包备份仍然是必要的
            </blockquote>
            <!-- 数据表格 -->
            ...
            <ul class="backup" style="margin-top:18px;display:none;padding-right: 10px;">
                <form class="layui-form" lay-filter="backup">
                    <div class="layui-form-item">
                        ...
                        <div class="layui-input-block">
                            <input type="text" name="desc" placeholder="您可以对本次备份的描述,也可以留空" value=""
                                   class="layui-input">
                        </div>
                    </div>
                    <div class="layui-form-item">
            ...
            <blockquote class="layui-elem-quote" style="margin-top: 30px;border-left: 5px solid #ff5858;">
                <a target="_blank" style="cursor:pointer;color: #ff5858;" rel="nofollow" id="data_empty">数据清空 ( 慎用
                    ) &nbsp;>>&nbsp; 不可逆,请提前备份! </a>
            </blockquote>
            <ul class="data_empty" style="margin-top:18px;display:none;padding-right: 10px;">
                <form class="layui-form layuimini-form" lay-filter="data_empty">
                    <blockquote class="layui-elem-quote"
                                style="border-left: 0px; color: #ff2e2e;text-align: center;background-color: #ffffff;padding: 0px">
                        <p>部分数据存在关联性</p>
                        <p>单独清空某个数据可能会造成意想不到的后果</p>
                        <p>例:清除分类数据后链接数据会因丢失分类信息而无法显示
                        <p>
                    </blockquote>
                    <div class="layui-form-item">
                    ...
                    <div class="layui-form-item">
                        ...
                        <div class="layui-input-block" style="#margin-left: 32px;">
                            ...
                            <div style="position: absolute;top:0px;right: 0px;">
                                <button class="layui-btn layui-btn-danger" lay-submit lay-filter="define_data_empty"
                                        style="display: inline-block;">确定清空
                                </button>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    layui.use(['layer', 'element', 'upload', 'form', 'table'], function () {
        var layer = layui.layer, $ = layui.$, upload = layui.upload, form = layui.form, table = layui.table,
        ...
        var up_bookmark = upload.render({
            ...
            , choose: function (obj) {
                ...
                if (res.code == 1) {
                    ...
                    layer.alert(res.msg || "上传异常,请刷新重试<br />若无法解决请联系技术支持", {
                        icon: 5,
                        title: '上传失败',
                        anim: 2,
                        closeBtn: 0,
                ...
                layer.alert("上传异常,请刷新重试<br />若无法解决请联系技术支持", {
                    icon: 5,
                    title: '错误',
                    anim: 2,
                    closeBtn: 0,
        ...
        $('#tip2').on('click', function () {
            layer.open({
                title: '温馨提示',
                scrollbar: false,
                content: '浏览器书签记录特性<br />1.链接标题可以为空,地址可以重复<br />2.文件夹名可空且可重复<br />3.支持无限子分类<br />本程序特性:<br />1.标题不能为空,链接不能重复<br />2.分类名不能为空且不能重复<br />3.仅支持二级分类<br />可能出现的问题:<br />1.书签所属的分类可能会错位(如2个相同文件夹的分类下的链接会合并到第一个)'
            });
        });
        ...
        $('#view_script').on('click', function () {
            ...
            layer.open({
                ...
                resize: false,
                title: '脚本预览',
                area: ($(window).width() < 768 ? '100%' : '768px'),
                content: html
            });
            return false;
        });
        ...
        $('#one-click-add').on('click', function () {
            layer.tips("按住鼠标左键将它拖拽到浏览器书签栏", "#one-click-add", {
                tips: [3, "#009688"],
                time: 10 * 1000,
                anim: 6
            });
        ...
        $('.export_data').on('click', function () {
            ...
            console.log(type);
            index = layer.prompt({
                formType: 1, value: '', title: '输入登录密码:', shadeClose: false, "success": function () {
                    //监听回车事件
        ...
        function export_data(type, name = null) {
            ...
            $.post(api_create, {
                'pwd': $.md5($("input.layui-layer-input").val()),
                'type': type,
                'name': name
                ...
                if (data.code == 1) { //如果导出成功
                    ...
                    let tip = layer.alert("请妥善保存您的数据!<br />请勿使用迅雷等第三方工具下载", {
                        icon: 1,
                        title: '导出成功',
                        offset: 'b',
                        anim: 2,
        ...
        table.render({
            ...
            , cols: [[
                ...
                , {
                    field: 'size', width: 80, title: '大小', templet: function (d) {
                        return bytesToSize(d.db_size + d.tar_size);
                    }
                }
                , {
                    width: 70, title: '分类', templet: function (d) {
                        return d.user_categorys.count;
                    }
                }
                , {
                    width: 80, title: '链接', templet: function (d) {
                        return d.user_links.count;
                    }
                }
