...
function write_sys_settings()
{
    ...
    if (!is_subscribe('bool')) {
        ...
        if (!empty($_POST['global_header'])) {
            $o_config['global_header'] = "";
            $filter = true;
        }
        if (!empty($_POST['global_footer'])) {
        ...
        if ($_POST['guestbook'] == 1) {
            $o_config['guestbook'] = 0;
            $filter = true;
        }
        if ($_POST['link_extend'] == 1) {
                ...
                if ($dh = opendir($dir)) {
                    while (false !== ($file = readdir($dh))) {
                        if ($file != "." && $file != "..") {
                            $fullpath = $dir . "/" . $file;
                            if (!is_dir($fullpath)) {
                                if ($time - filemtime($fullpath) > $minute * 60) {
                                    $_SESSION['CleanCacheSize'] += filesize($fullpath);
                                    unlink($fullpath);
                                }
                            } else {
                                delfile($fullpath, $minute);
